...
from api.v2.utils import get_distance
...
def add_ride():
    ...
    required = ["origin", "destination", "date_time", "available_seats"]
...
def update_ride(ride_id):
    ...
    """
    updateable = ["origin", "destination", "distance", "date_time", "available_seats"]
    driver_id = get_jwt_identity()
    ride = Ride.objects(id=ride_id).first()
    if not ride:
        return jsonify({"error": "Ride not found"}), 404
...
def search_rides():
    ...
    rides = Ride.objects(
        origin=origin_regex, destination=destination_regex, date_time__gte=date_time
    )
